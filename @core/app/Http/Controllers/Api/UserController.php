...
class UserController extends Controller
{
    ...
    public function profile(){
        ...
        $user = User::with('country:id,name')->select('id','name','email','phone','image','zipcode','city','country_id','district','address','state')
    ...
    public function user_donations()
{
    ...
    $donationsPaginator = CauseLogs::with('cause:id,title,title_ar')
        ->select('id','status','cause_id','amount','price','totalPrice','payment_gateway','created_at','address') // ✅ فقط address
        ->where('user_id', $user->id)
        ->orderBy('id', 'DESC')
        ->paginate(10)
        ->withQueryString();

    $donations = $donationsPaginator->toArray();
