...
class CausesLogController extends Controller
{
    ...
    'address.required' => 'حقل العنوان مطلوب',
        ...
        //Checking campaigns are owner campaigns or not
            ...
            $num = $cause->amount - $cause-> raised;
            if($num < $request->amount){
                return back()->with(['msg' => __( 'لقد تجاوزت كمية الطلب المتاحة (الحد الأقصى: ' . $num . ').'), 'type' => 'danger']);
        ...
        if (!empty($request->order_id)) {
          ...
          //Custom Fields Code
            $payment_log_id = CauseLogs::create([
                ...
                'phone'=>$request->phone,
                 'address' => 
                    __('City') . ': ' . $request->city . '<br>' .
                    __('District') . ': ' . $request->district . '<br>' .
                    __('Address') . ': ' . $request->address,
                ...
                'price'=>$request->price,
                'totalPrice'=>$request->price * $amount,
            ])->id;
