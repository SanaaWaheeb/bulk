    ...
    public function donations(Request $request)
{
    ...
    $all_donations = $query
        ...
        ->orderByRaw("
            ...
            END ASC
        ")
        /* 1) غير المكتملة أولاً، ثم المكتملة */
        ->orderByRaw("
            CASE 
                WHEN COALESCE(amount,0) = 0 THEN 1
                WHEN COALESCE(raised,0) < COALESCE(amount,0) THEN 0
                ELSE 1
            END ASC
        ")
        /* 2) داخل غير المكتملة: الأقل تقدماً أولاً */
        ->orderByRaw("
            CASE 
                WHEN COALESCE(amount,0) = 0 THEN 1
                ELSE COALESCE(raised,0) / NULLIF(COALESCE(amount,0),0)
            END ASC
        ")
