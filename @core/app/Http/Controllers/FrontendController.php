...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $feature_cause = Cause::query()
        ->where(['status' => 'publish','featured' => 'on'])
        ->orderByRaw('CASE WHEN COALESCE(raised,0) < COALESCE(amount,0) THEN 0 ELSE 1 END ASC')
        ->orderByRaw('CASE WHEN NULLIF(amount,0) IS NULL THEN 1 ELSE (COALESCE(raised,0)/NULLIF(amount,0)) END ASC')
        ->orderBy('id','desc')
        ->get();
        ...
        $all_recent_donation = Cause::select('title','title_ar','amount','cause_content','raised','image','slug','categories_id','excerpt','featured','id','reward','price')
        ->where(['status' => 'publish'])
        ->orderByRaw('CASE WHEN COALESCE(raised,0) < COALESCE(amount,0) THEN 0 ELSE 1 END ASC') // غير المكتملة أولاً
        ->orderBy('id','desc');
...
//        }
        ...
        $all_recent_donation = Cause::select('title','title_ar','amount','cause_content','raised','image','slug','categories_id','excerpt','featured','id','reward')->where(['status' => 'publish'])->orderBY('id','desc');
