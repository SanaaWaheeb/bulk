   ...
   public function store_donation(Request $request)
{
    ...
    // 🔥 استبدل كل كود الـ specifications بهذا الكود البسيط:
    $specifications = [];
...
public function update_donation(Request $request)
{
    ...
    $slug = !empty($request->slug) ? Str::slug($request->slug) : Str::slug($request->title);
    $slug_check = Cause::where('id', '!=', $request->donation_id)->where(['slug' => $slug])->count();
    $cause_slug = $slug_check > 0 ? $slug.'-'.($slug_check + 1) : $slug;
    ...
    if ($request->has('specifications') && !empty($request->specifications)) {
        try {
            ...
            // في حالة خطأ في JSON، تخزين كصفيف فارغ
            $specifications = [];
        }
    }
     $specifications_en = [];
    ...
    Cause::findOrFail($request->donation_id)->update([
        ...
        'monthly_donation_status' => $request->monthly_donation_status ?? 0,
        'gift_status' => $request->gift_status ?? 0,
        'emmergency_title' => $request->emmergency_title,
        'og_meta_title' => $request->og_meta_title,
        'og_meta_description' => $request->og_meta_description,
        'og_meta_image' => $request->og_meta_image,
    ...
    public function change_status(Request $request)
    {
        ...
        $oldStatus=$order_details->status;
       ...
       // If status is changing from complete or pending → to cancel → subtract
        if (
            ($oldStatus == 'pending' && $status == 'cancel') ||
            ($oldStatus == 'complete' && $status == 'cancel')
        ) {
            $event_details->raised = (int)$event_details->raised - (int)$order_details->amount;
            $event_details->save();
        }
        // If status is changing from cancel → to pending or complete → add
        elseif (
            ($oldStatus == 'cancel' && $status == 'pending') ||
            ($oldStatus == 'cancel' && $status == 'complete')
        ) {
            $event_details->raised = (int)$event_details->raised + (int)$order_details->amount;
            $event_details->save();
        }
        // Else → do nothing
            ...
            // $data['subject'] = __('تم تغيير حالة طلبك إلى: ') 
            //     . '(' . ucfirst($status) . ')' 
            //     . ' في ' . get_static_option('site_title');
            
            // $data['message'] = __('مرحباً') . ' ' . $order_details->name . '<br>';
            // $data['message'] .= __('تم تغيير حالة طلبك إلى: ') 
            //     . '(' . ucfirst($status) . ')' . '<br>';
            ...
            // $data['message'] .= ' <a href="' 
            //     . route('user.home') 
            //     . '">' . __('لوحة التحكم الخاصة بك') . '</a>';
            
            // try {
            //     Mail::to($user_mail)->send(new BasicMail($data));
            // } catch (\Exception $e) {
            //     return redirect()->back()
            //         ->with(['msg' => $e->getMessage(), 'type' => 'danger']);
            // }
            
            // return redirect()->back()
            //     ->with([
            //         'msg'  => __('تم تغيير حالة الطلب بنجاح.'),
            //         'type' => 'success'
            //     ]);
    ...
    public function donated_donors($id){
        ...
        if (\request()->ajax()){
            $donation_logs = CauseLogs::where('cause_id', $id)
