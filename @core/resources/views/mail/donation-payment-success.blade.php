...
<div class="mail-container">
    ...
    <div class="main-content-wrap">
        <p>{{__('Hello')}}</p>
        @if($type == 'customer')
        <p>{{__('A payment from')}} {{($data->cause)->title_ar}} {{__('was successful. Donation Log ID')}} #{{$data->id}} </p>
            <br>
            @if(!empty($data->gift))
                <strong>{{__('(You will get donation gift very soon according your provided address)')}}</strong>
            @endif
        @else
            <p>{{__('You get payment from')}} {{$data->name}} {{__('For donation log ID') .'#'}} {{$data->id}}, {{__('donated cause')}} {{optional($data->cause)->title}} </p>
        @endif
        <div class="price-wrap">{{amount_with_currency_symbol($data->totalPrice)}}</div>
        <table>
            <tr>
                <td>{{__('Donate ID')}}</td>
                <td>#{{$data->id ?? ''}}</td>
            ...
            <tr>
                <td>{{__('Cause Name')}}</td>
                <td>{{optional($data->cause)->title_ar ?? ''}}</td>
            </tr>
            <tr>
                <td>{{__('Phone')}}</td>
                <td>0{{$data->phone}}</td>
            ...
            <tr>
                <td>{{__('TotalPrice')}}</td>
                <td>{{amount_with_currency_symbol($data->totalPrice)}}</td>
            </tr>
            <!--<tr>-->
