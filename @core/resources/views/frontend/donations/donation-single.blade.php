                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="contribute-single-page-item single-flag-contribute">
                            ...
                            @endif
                            {{-- <div class="product-title-section margin-top-30">
                                <h1 class="product-main-title">{{ $donation->title_ar }}</h1>
                            </div> --}}
...
<div class="section-title ">
    ...
    <div class="donation-details" id="donate_box_wrapper">
        <div class="amount-details">
            <h3 class="price">{{__('Price')}}   {{amount_with_currency_symbol($donation->price)}}</h3>
        </div>
    </div>
    ...
    <div class="btn-wrapper margin-top-30">
        ...
        @endif
                                ...
                                {{-- Donate in Seperate Page --}}
                                    ...
                                    @if(!empty(get_static_option('donation_page_qr_code_show_hide')))
                                    <div class="social-texarea-share mt-4">
                                        <h4 class="share-title">{{ get_static_option('donation_page_qr_code_title') ?? __('QR code') }}</h4>
                                        <div class="content">
                                            {!! $qr_code !!}
                                            <br>
                                            <div class="btn-wrapper mt-3">
                                                <a href="{{ route('frontend.donations.qrcode.download', $donation->id) }}" class="boxed-btn btn-sm reverse-color">{{ __('Download QR Code') }}</a>
                                            </div>
                                        </div>
                                    </div>
                                    @endif
    ...
    $(document).on('click', '#ReadMoreButton', function (e) {
        ...
        $(this).parent().remove(); // remove the button
                    ...
                    $(document).on('click', '#load_more_comment_button', function () {
                        $('#load_more_comment_button').html('<b>{{__('تحميل...')}}</b>');
                        load_comment_data('{{$donation->id}}', _token);
                    });
                    ...
                    $(document).on('click', '#load_more_case_update_button', function () {
                        $('#load_more_case_update_button').html('<b>{{__('تحميل...')}}</b>');
                    ...
                    function load_donation_update(id){
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                if (data.length < 1) {
                                    appendData += '<p class="not-found-button">{{__('لم يتم العثور على المزيد من التحديثات')}}</p>';
                                } else {
                                    appendData += '<div class="btn-wrapper load_more"> <button type="button" class="load-more-btn" id="load_more_case_update_button">{{__('تحميل المزيد')}}</button> </div>';
                                }
                    ...
                    $(document).on('click', '#load_more_button', function () {
                        $('#load_more_button').html('<b>{{__('تحيل...')}}</b>');
